...
class DThread(QThread):
    ...
    def run(self):
        ...
        if (size_of_table == 0):
            ...
            dig = self.detect_digits(size_of_table, temp_output, field_names)
            if dig == 0:
                msg = EmailMessage()
                msg.set_content(f"ALERT: Your machine might be failing at stream {self.label_id+1}")
                msg['Subject'] = 'Machine failure'
                msg['From'] = "psht5910@gmail.com"
                msg['To'] = email_file.file_text
                s = smtplib.SMTP('smtp.gmail.com', 587)
                s.starttls()
                s.login('psht5910@gmail.com', 'fnpg vqmm bhsf iihu')
                s.send_message(msg)
                s.quit()
